...
-- package body Arch.Entrypoint is
 --  procedure Bootstrap_Main is
  ...
  -- begin
      ...
      -- Commenting out the Reboot procedure call and its handling
      -- Arch.Power.Reboot(Status);

      -- if Status = Failure then
      --    Lib.Messages.Put_Line ("Reboot failed.");
      -- else
      --    Lib.Messages.Put_Line ("Reboot succeeded.");
      -- end if;
...
--      loop null; end loop;
      ...
      --pragma Compile_Time_Error
       -- (Condition => KASAN_Supported,
       --  Message => "KASAN is not supported in this build");
...
end Arch.Entrypoint;--  arch-entrypoint.adb: Limine plops us here.
--  Copyright (C) 2024 streaksu
--
--  This program is free software: you can redistribute it and/or modify
--  it under the terms of the GNU General Public License as published by

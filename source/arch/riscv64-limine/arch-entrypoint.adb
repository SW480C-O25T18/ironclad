...
package body Arch.Entrypoint is
   ...
   ------------------------------------------------------------------------
   -- Bootstrap_Main
   --   Entry point invoked by Limine. Performs early hardware setup:
   --     • UART0 init for debug
   --     • Limine protocol translation
   --     • DTB parsing
   --     • Physical allocator & MMU init
   --     • Logging/KASAN
   --     • Memory map printout
   --     • CPU cores init
   ...
   begin
      -- UART0 for early debug (must handle return value in Ada)
      ...
      -- 1. Limine protocol → arch‐agnostic
      ...
      -- 4. Enable logging (and KASAN if configured)
      ...
      if CLINT_Node /= null then
         ...
         begin
            if CLINT_Reg'Length >= 4 then
               ...
               Arch.Debug.Print("CLINT DTB info incomplete; using defaults.");
               Arch.CLINT.Set_CLINT_Configuration;
            end if;
         end;
      else
         Arch.Debug.Print("CLINT node not found; using defaults.");
         Arch.CLINT.Set_CLINT_Configuration;
      end if;

      -- 8. Configure PLIC from DTB
      ...
      if PLIC_Node /= null then
         ...
         begin
            ...
            if PLIC_Reg'Length >= 2 then
               ...
               Arch.Debug.Print("PLIC configured from DTB and SMP info.");
            else
               Arch.Debug.Print("PLIC DTB info incomplete; using defaults.");
               Arch.PLIC.Set_PLIC_Configuration;
            end if;
         end;
      else
         Arch.Debug.Print("PLIC node not found; using defaults.");
         Arch.PLIC.Set_PLIC_Configuration;
      end if;
      ...
      -- 10. Set trap vector to Arch.Interrupts.trap_entry
